...
export default function HomePage() {
  ...
  useEffect(() => {
    const checkAuthAndFetchData = async () => {
      try {
        ...
        // APIエラーの場合はデモデータを表示
        console.log('Using demo data for home stats due to API error');
        setHomeData({
          averageSatisfaction: 2.5,
          yesterdaySatisfaction: 3,
          consecutiveDays: 5
        });
      } finally {
        setLoading(false);
  ...
  const averageSatisfaction = homeData?.averageSatisfaction ?? 2;
        ...
        <Card className="border-orange-200 bg-white/90 max-w-md mx-auto">
          <CardContent className="p-4 text-center">
            ...
            <div className="text-sm text-gray-600 mt-2">
              数値: {averageSatisfaction.toFixed(1)} / 4.0
            </div>
          </CardContent>
        </Card>
      </div>
